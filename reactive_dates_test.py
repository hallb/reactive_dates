...
class TestDateFunction(unittest.TestCase):
    ...
    def test_or(self):
        ...
        actual_orL = reactive_dates.orL(
            reactive_dates.weeklyL(d(2012, 1, 1)),
            reactive_dates.weeklyL(d(2012, 1, 3)))
        actual = list(filter(actual_orL, reactive_dates.daily(r)))
        expected = [
            d(2012, 1, 1),
            d(2012, 1, 3),
            d(2012, 1, 8),
    ...
    def test_on(self):
        r = (d(2011, 12, 31), d(2012, 1, 14))
        actual_onL = reactive_dates.onL(d(2012, 1, 1))
        actual = list(filter(actual_onL, reactive_dates.daily(r)))
        expected = [d(2012, 1, 1)]
        self.assertEqual(actual, expected)
